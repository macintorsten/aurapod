...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    rssService.getTrendingPodcasts()
  ...
  const handleImportDirect = useCallback(async (data: SharedData) => {
    if (data.t && data.u) {
      const podcastId = data.p ? btoa(data.p).substring(0, 16) : APP_CONSTANTS.STANDALONE_PODCAST_ID_PREFIX;
  ...
  const subscribePodcast = useCallback((podcast: Podcast) => {
    addPodcast(podcast);
  ...
  const isSubscribed = useCallback((feedUrl: string) => 
    podcasts.some(p => p.feedUrl === feedUrl),
    [podcasts]
  );

  const handlePlayFromHistory = useCallback(async (item: PlaybackState) => {
  ...
  const generateShareData = useCallback((podcastUrl: string, episodeId?: string) => {
    const pod = podcasts.find(p => p.feedUrl === podcastUrl) || activePodcast;
    const ep = episodes.find(e => e.id === episodeId) || (currentEpisode?.id === episodeId ? currentEpisode : null);
  ...
  const playerPodcast = useMemo(() => {
    ...
    return podcasts.find(p => p.id === currentEpisode.podcastId) || 
           activePodcast || 
           {
