...
interface AppContextValue {
  ...
  updateHistory: (episode: Episode, podcast: Podcast | { title: string }, currentTime: number, duration: number) => void;
  ...
  // Error management
  errors: AppError[];
  addError: (category: AppError['category'], message: string, error?: any, context?: any) => void;
  clearErrors: () => void;
}
...
export function AppProvider({ children }: AppProviderProps) {
  ...
  // Add error handler
  const addError = useCallback((category: AppError['category'], message: string, error?: any, context?: any) => {
    const errorObj = createError(category, message, error, context);
    setErrors(prev => [errorObj, ...prev].slice(0, APP_CONSTANTS.MAX_ERRORS));
    logError(errorObj);
  }, []);
  ...
  const loadPodcast = useCallback(async (podcast: Podcast) => {
    setLoading(true);
    setActivePodcast(podcast);
    
    try {
      const { episodes: fetchedEpisodes } = await rssService.fetchPodcast(podcast.feedUrl);
      setEpisodes(fetchedEpisodes);
    } catch (err) {
      addError('feed', `Failed to load podcast: ${podcast.title}`, err, { feedUrl: podcast.feedUrl });
      setEpisodes([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadNewEpisodes = useCallback(async () => {
    setLoading(true);
    const allNewEpisodes: (Episode & { podcastTitle: string; podcastImage: string })[] = [];
    ...
    for (const podcast of podcasts) {
      try {
        ...
        addError('feed', `Failed to load new episodes for ${podcast.title}`, err);
      }
    }
    
    // Sort by date
    allNewEpisodes.sort((a, b) => new Date(b.pubDate).getTime() - new Date(a.pubDate).getTime());
