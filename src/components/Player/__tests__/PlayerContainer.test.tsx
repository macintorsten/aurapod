...
describe("PlayerContainer", () => {
  ...
  it("saves progress when toggling pause", async () => {
    const user = userEvent.setup();
    const updatePlaybackSpy = vi.spyOn(
      storageService.storageService,
      "updatePlayback"
    );
  ...
  it("shows queue button when queue has items", () => {
    render(<PlayerContainer {...defaultProps} queue={[createMockEpisode()]} />);
  ...
  it("displays error message when audio URL is missing", () => {
    ...
    render(<PlayerContainer {...defaultProps} episode={episodeWithoutAudio} />);
