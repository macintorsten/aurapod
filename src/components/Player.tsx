...
const Player: React.FC<PlayerProps> = ({
    ...
    (seconds: number) => {
      if (isCasting) {
        const newTime = Math.max(
          0,
          Math.min(
            castService.getDuration(),
            castService.getCurrentTime() + seconds
          )
        );
  ...
  useEffect(() => {
    ...
    const unsubscribeState = castService.onStateChange(
      (isConnected, deviceName) => {
        ...
        if (isConnected && audioRef.current) {
          ...
          castService
            .loadMedia(episode, podcast, currentTime)
            .then(() => {
              setIsPlaying(true);
            })
            .catch((err) =>
  ...
  useEffect(() => {
    ...
    if (isCasting) {
      ...
      castService
        .loadMedia(episode, podcast, startTime)
        .then(() => {
          setIsPlaying(true);
      ...
      if (isNewEpisode) {
        ...
        const state = historyData[episode.id];
        audioRef.current.currentTime =
          state && !state.completed ? state.currentTime : 0;
