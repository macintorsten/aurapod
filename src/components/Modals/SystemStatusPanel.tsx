...
export const SystemStatusPanel: React.FC<SystemStatusPanelProps> = ({
  ...
  return (
    <div
      className="fixed inset-0 z-[200] bg-zinc-950/60 backdrop-blur-xl flex items-center justify-center p-6"
      onClick={onClose}
    >
      <div
        className="bg-white dark:bg-zinc-900 w-full max-w-2xl rounded-[3rem] p-10 shadow-3xl animate-fade-in flex flex-col max-h-[80vh]"
      ...
      >
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={onClose}
            className="w-10 h-10 flex items-center justify-center bg-zinc-100 dark:bg-zinc-800 rounded-full text-zinc-400 hover:text-white transition"
          >
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pr-2">
          {errors.length === 0 ? (
            <div className="text-center py-20 text-zinc-400 font-medium italic">
              No signal interruptions detected.
            </div>
          ) : (
            errors.map((err, i) => (
              <div
                key={i}
                className="p-6 bg-zinc-50 dark:bg-zinc-800/40 rounded-[2rem] border border-zinc-200 dark:border-zinc-800/60"
              >
                <div className="flex items-center justify-between mb-2">
                  <span
                    className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${
                      err.category === "playback"
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`text-[9px] font-bold transition flex items-center gap-1.5 ${
                        copiedIndex === i
                          ? "text-green-500"
                    ...
                    >
                      <i
                        className={`fa-solid ${
                          copiedIndex === i ? "fa-check" : "fa-copy"
                        }`}
                      ></i>
                      {copiedIndex === i
                        ? "Copied to buffer!"
        ...
        <button
          ...
          className="mt-8 py-4 bg-zinc-100 dark:bg-zinc-800 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition"
        >
          Clear Activity Log
        </button>
      </div>
