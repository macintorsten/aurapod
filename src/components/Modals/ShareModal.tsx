      ...
      >
        ...
        <div className="flex-1 overflow-y-auto space-y-8 pr-2">
            ...
            {data.t && (
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-extrabold text-zinc-900 dark:text-white mb-1 truncate">
                    {data.t}
                  </p>
                  <p className="text-[10px] text-indigo-500 font-bold">
                    {data.st || "Standalone Broadcast"}
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            <button
              onClick={handleCopy}
              className={`w-full py-4 rounded-2xl font-bold text-xs transition flex items-center justify-center gap-3 ${
                copied
            ...
            >
              {copied ? (
                <>
                  <i className="fa-solid fa-check"></i> Link Copied
                </>
              ) : (
                <>
                  <i className="fa-solid fa-copy"></i> Generate Broadcast URL
                </>
              )}
            </button>
          </div>
        ...
        <div className="mt-8 bg-indigo-500/5 dark:bg-indigo-900/10 rounded-2xl p-4 flex gap-4 items-center">
          ...
          <p className="text-[10px] text-zinc-500 dark:text-zinc-400 leading-snug">
            This wave uses <b>Deflate compression</b> to embed full track
            metadata instantly with zero handshake required.
          </p>
        </div>
      </div>
    </div>
